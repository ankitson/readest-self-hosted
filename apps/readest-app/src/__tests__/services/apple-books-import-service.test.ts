...
const makeExport = () => ({
  ...
  book: {
    assetId: 'asset-1',
    title: "It's Not Always Depression: Working the Change Triangle",
...
describe('parseAppleBooksAnnotationsExport', () => {
  it('parses the versioned Apple Books annotations interchange format', () => {
    ...
    expect(parsed?.book.title).toContain("It's Not Always Depression");
  ...
  it('accepts punctuation and subtitle differences', () => {
    expect(
      titlesLikelyReferToSameBook(
        ...
        "It's Not Always Depression: Working the Change Triangle",
      ),
    ).toBe(true);
  });

  it('rejects a different book', () => {
    expect(titlesLikelyReferToSameBook('The Dispossessed', 'A City on Mars')).toBe(false);
  });
});
